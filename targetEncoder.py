...
class Normalization(nn.Module):
    def __init__(self, embedding_dim):
        ...
        self.normalizer = nn.LayerNorm(embedding_dim, elementwise_affine=True)
...
class TargetEncoder(nn.Module):
    def __init__(self, cfg):
        ...
        self.idle_embedding = nn.Parameter(torch.Tensor(1,1,self.embedding_dim))
        torch.nn.init.uniform_(self.idle_embedding, 0, 1)

        self.init_embedding = nn.Linear(2, self.embedding_dim)
        self.layers = nn.Sequential(*(
            MultiHeadAttentionLayer(embedding_dim=self.embedding_dim)
            for _ in range(3)
        ))  # * can capture the elements in the iteration

    def forward(self, target_input, idle_embedding = None,  mask=None):
        assert mask is None, "mask is None"

        if idle_embedding is not None:
            h = torch.cat((idle_embedding, self.init_embedding(target_input)), dim=1)
            h = self.layers(h)
            return h # batch_size*targets_size*embedding_dim
        else:
            h = torch.cat((self.idle_embedding, self.init_embedding(target_input)), dim=1)
            h = self.layers(h)
            return h, self.idle_embedding.detach() # batch_size*targets_size*embedding_dim
